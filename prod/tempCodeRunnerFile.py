...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Dropdown(id="plot-type", style={'width': '50%', 'margin': 'auto'},
...
def update_visualization(selected_column, contents, remove_duplicates, fill_na):
    if contents is not None and selected_column is not None:
        ...
        decoded = base64.b64decode(content_string)
        data = pd.read_csv(io.StringIO(decoded.decode('utf-8')))
        print(f"Data after loading: {data}")
        ...
        if fill_na is not None:
            data = data.fillna(fill_na)
            print(f"Data after filling NA values: {data}")

        print(f"Data before visualization: {data}")
        return {
...
@app.callback(Output("summary-stats", "children"), [Input('upload-data', 'contents'), Input('remove-duplicates', 'value'), Input('fill-na', 'value')])
...
@app.callback(
    ...
    [State('upload-data', 'contents'), State('remove-duplicates', 'value'), State('fill-na', 'value')],
